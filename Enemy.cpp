...
bool Enemy::CheckPlayerCollision(const Player& player)
{
	POINT check_position = {
		position.x + FRAME_WIDTH / 2, position.y + FRAME_HEIGHT / 2
	};
	...
	//bool is_overlap_x = player.GetPosition().x >= position.x
	//	&& player.GetPosition().x <= position.x + FRAME_WIDTH;

	//bool is_overlap_y = player.GetPosition().y >= position.y
	//	&& player.GetPosition().y <= position.y + FRAME_HEIGHT;
